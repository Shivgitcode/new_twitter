...
export default function Home() {
  ...
  const likeHandler = async (id: string) => {
    ...
    const response = await fetch(
      `${import.meta.env.VITE_API_URL}/likes/${id}`,
      {
        method: "POST",
        credentials: "include",
        headers: {
    ...
    <div className=" w-full h-full">
      <section className=" w-[30%] mx-auto border-[0.2px] border-b-0 border-[#e7e9ea9f]">
        <div className="flex flex-col items-start border-b-[0.5px]">
          <h1 className="text-white text-[18px] font-bold leading-7 p-[16px] w-full">
            Home
          </h1>
          <form className="w-full p-[16px]" onSubmit={handleSubmit}>
            <div className="flex justify-start items-start w-full">
              ...
              <div className="w-full flex flex-col ">
                <textarea
                  ...
                  value={tweet}
                  rows={3}
                  className=" bg-transparent text-white w-full p-[8px] placeholder:text-[#e7e9ea9f] focus:outline-none"
                ></textarea>
                ...
                <div className={`w-[108px] ${preview && "min-h-3"} mb-1`}>
            ...
            <div className="w-full flex justify-end py-2">
              <button className=" text-white rounded-3xl px-[20px] py-[4px] bg-[#308cd8] ml-auto">
                Tweet
              </button>
            </div>
          </form>
        ...
        <div className="flex flex-col items-start">
          {comments?.map((el) => (
            <div
              key={el.id}
              className="flex justify-start items-start w-full border-b-[1px] p-[20px]"
            >
              ...
              <div>
                <div className=" text-white">
                  <span className="uppercase mr-[4px]">{el.user.username}</span>
                  <span className=" text-[#71767b]">
                    @{el.user.email.slice(0, 10)}{" "}
                    {`${
                      months[new Date(el.timeStamp).getUTCMonth()]
                    } ${new Date(el.timeStamp).getUTCDate()}`}
                  </span>
                </div>
                ...
                <div className="flex w-full justify-between mt-3">
                  <div className="mr-[4px] text-[#71767b]">
                    ...
                    >
                      <VscComment fill="white"></VscComment>
                      <span>{el.comments.length}</span>
                    </Link>
                  </div>
                  <div className="mr-[4px] text-[#71767b] flex items-center justify-center gap-1">
                    <BiRepost fill="white"></BiRepost>
                    <span>0</span>
                  </div>
                  <div
                    className="mr-[4px] text-[#71767b] flex items-center justify-center gap-1"
                    onClick={() => {
                      likeHandler(el.id);
                    }}
                  >
                    ...
                    {handleLike(el.id) ? (
                      ...
                      <FaRegHeart fill="white"></FaRegHeart>
                    )}
                    <span>{el.likedBy.length}</span>
                  </div>
                  <div className="mr-[4px] text-[#71767b] flex items-center justify-center gap-1">
                    <IoShareSocialOutline stroke="white"></IoShareSocialOutline>
                    <span>0</span>
                  </div>
                </div>
              </div>
      ...
      <div className="w-full flex justify-center py-5">
        <button
          className="py-[10px] px-[20px] rounded-full bg-white "
